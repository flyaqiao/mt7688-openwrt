...
unsigned short CalcCRC16(unsigned char *Dat,  unsigned int len)
{
  unsigned short i, TxCRC16;
  TxCRC16 = 0;
  for (i = 0; i < len; i++)
...
unsigned char CalcCRC8(unsigned char *p, char counter)
{
...
{
  unsigned long CRC32, dtmp;
  int di;
  int i;
  CRC32 = 0xFFFFFFFF ^ CRCinit;
  for (di = 0; di < dlen; di++) {
    dtmp = buf[di] & 0xFF;
    for (i = 0; i < 8; i++) {
      if (((CRC32 ^ dtmp) & 1) != 0)
        CRC32 = ((CRC32 >> 1) & 0x7FFFFFFF) ^ poly;
      else
        CRC32 = ((CRC32 >> 1) & 0x7FFFFFFF);
      dtmp = (dtmp >> 1) & 0x7FFFFFFF;
    }
  }
