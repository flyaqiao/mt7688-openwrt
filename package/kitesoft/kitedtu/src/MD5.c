...
* All rights reserved. 
*  
* 文件名称：$HeadURL: https://codeserver/svn/KnitSystem/NewSystem/KnitSystem/MD5.cpp $ 
* 摘    要：
*  
* 当前版本：$Revision: 1861 $
...
int MD5String(const char* inStr, char* outStr, int outLen)
{
  ...
  memset(output1, 0, 34);
  MD5Init(&context);
  MD5Update(&context, (unsigned char*)inStr, len);
  ...
  //}
  output1[32] = 0;
  strcpy_s(outStr, outLen, output1);
...
#if 0
int MD5String(const TCHAR* inString, TCHAR* outString, int outLen)
{
  //char inStr[MAX_PATH];
  char outStr[MAX_PATH];
  //memset(inStr, 0, MAX_PATH);
  memset(outStr, 0, MAX_PATH);
  int len = WideCharToMultiByte(CP_ACP, 0, inString, -1, NULL, 0, NULL, NULL);
  char* inStr = new char[len + 1];
  memset(inStr, 0, len + 1);
  WideCharToMultiByte(CP_ACP, 0, inString, -1, inStr, len, NULL, NULL);
  int iRet = MD5String(inStr, outStr);
  delete[] inStr;
  wmemset(outString, 0, outLen);
  MultiByteToWideChar(CP_ACP, 0, outStr, -1, outString, outLen);
  return iRet;
}
...
{
  char szMD5[34];
  MD5String(string, szMD5, 34);
  return strncmp(_strupr(md5string), _strupr(szMD5), 32) == 0;
} 
...
BOOL MD5Check(LPCTSTR md5string, LPCTSTR string)
{
  ...
  WideCharToMultiByte(CP_ACP, 0, md5string, -1, md5, MAX_PATH, NULL, NULL);
...
{
  MD5_CTX context;
  unsigned int digest[4] = { 0 };
  MD5Init(&context);
  MD5Update(&context, inBuf, inLen);
  MD5Final((unsigned char*)digest, &context);
  if (digest[0] != md5[0] || digest[1] != md5[1] || digest[2] != md5[2] || digest[3] != md5[3])
    return 0;
  return 1;
}
